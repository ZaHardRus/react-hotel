...
export const LikeButton: FC<LikeButtonProps> = ({width = 21, height = 18, _id, onClick}) => {
    const {favorites} = useAppSelector(state => state.hotel)
    const isFavorite = favorites.some(el => el.new_id === _id)

    return (
        <div className={style.likeButton} onClick={onClick}>
            <svg width={width} height={height} viewBox="0 0 23 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    ...
                    fill={isFavorite ? 'red' : 'white'} stroke={isFavorite ? 'red' : '#c4c4c4'} strokeLinecap="round"
